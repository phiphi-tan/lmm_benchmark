...
def prep_data(ds_path, ds_split, split_size=None):
    ...
    # get either author / title
    random.seed(split_size)
    qn_choice = [random.randint(0,1) for _ in range(split_size)]

    question_data_list = input_dataset['questions'] # get list of questions (if necessary)
    question_data_list = [question_data_list[i][qn_choice[i]] for i in range(split_size)]

    answer_data_list = input_dataset['answers'] # get list of answers
    answer_data_list = [answer_data_list[i][qn_choice[i]] for i in range(split_size)]
